...
class LicenseChainConfig(BaseModel):
    ...
    base_url: str = Field(default="https://api.licensechain.app/v1", description="Base URL for the API")
...
class LicenseChainClient:
    ...
    async def update_user_profile(self, attributes: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        raise ValidationError("User profile update endpoint is not available in API v1")
    ...
    async def create_license(
        ...
        return await self._post(f"/apps/{app_id}/licenses", payload)
    ...
    async def get_license(self, license_id: str) -> Dict[str, Any]:
        ...
        The API supports both license ID and license key for lookup.
        
        Args:
            license_id: License ID or license key
            
        Returns:
            License data including:
            - id: License ID
            - key: License key
            - plan: License plan (FREE, PRO, BUSINESS, ENTERPRISE)
            - status: License status (ACTIVE, EXPIRED, REVOKED, SUSPENDED)
            - issuedTo: Name of the person/license holder
            - issuedEmail: Email address
            - email: Email address (alias for issuedEmail)
            - expiresAt: Expiration date (ISO string or null)
    ...
    async def get_analytics(
        ...
        """
        params = {}
        if metric in ("7d", "30d", "90d", "1y"):
            params["period"] = metric
        return await self._get("/analytics/stats", params)
    ...
    async def get_dashboard_insights(self) -> Dict[str, Any]:
        ...
        """
        return await self._get("/analytics/stats")
    ...
    async def list_products(
        ...
        """
        raise ValidationError("Seller product endpoints are not available in API v1")
    ...
    async def get_product_analytics(self, product_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        """
        raise ValidationError("Seller analytics endpoints are not available in API v1")
    ...
    async def list_teams(self) -> Dict[str, Any]:
        ...
        """
        raise ValidationError("Team endpoints are not available in API v1")
